...
class TrendStatus(Enum):
    """Trend status enum"""
    STRONG_BULL = "Strong Bullish"      # MA5 > MA10 > MA20, spread widening
    BULL = "Bullish Alignment"          # MA5 > MA10 > MA20
    WEAK_BULL = "Weak Bullish"          # MA5 > MA10, but MA10 < MA20
    CONSOLIDATION = "Consolidation"     # MAs coiling
    WEAK_BEAR = "Weak Bearish"          # MA5 < MA10, but MA10 > MA20
    BEAR = "Bearish Alignment"          # MA5 < MA10 < MA20
    STRONG_BEAR = "Strong Bearish"      # MA5 < MA10 < MA20, spread widening


class VolumeStatus(Enum):
    """Volume status enum"""
    HEAVY_VOLUME_UP = "High-Volume Rally"         # price and volume rising together
    HEAVY_VOLUME_DOWN = "High-Volume Drop"        # volume surge on decline
    SHRINK_VOLUME_UP = "Low-Volume Rise"          # rising on low volume
    SHRINK_VOLUME_DOWN = "Shrink-Volume Pullback" # shrinking volume pullback (positive)
...
class BuySignal(Enum):
    ...
    HOLD = "Hold"                # already holding, continue
    WAIT = "Watch"               # wait for better opportunity
...
class MACDStatus(Enum):
    """MACD status enum"""
    GOLDEN_CROSS_ZERO = "Golden Cross Above Zero"  # DIF crosses DEA above zero axis
    GOLDEN_CROSS = "Golden Cross"                  # DIF crosses above DEA
    BULLISH = "Bullish MACD"                       # DIF > DEA > 0
    CROSSING_UP = "DIF Crossing Up Zero"           # DIF crosses above zero axis
    CROSSING_DOWN = "DIF Crossing Down Zero"       # DIF crosses below zero axis
    BEARISH = "Bearish MACD"                       # DIF < DEA < 0
...
class RSIStatus(Enum):
    ...
    STRONG_BUY = "RSI Strong"    # 50 < RSI < 70
...
class StockTrendAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, code: str) -> TrendAnalysisResult:
        ...
        if df is None or df.empty or len(df) < 20:
            ...
            result.risk_factors.append("Insufficient data, analysis incomplete")
            return result
    ...
    def _analyze_trend(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if ma5 > ma10 > ma20:
            ...
            result.ma_alignment = "MAs coiling, trend unclear"
    ...
    def _analyze_volume(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if result.volume_ratio_5d >= self.VOLUME_HEAVY_RATIO:
            if price_change > 0:
                ...
                result.volume_trend = "High-volume rally, strong bullish momentum"
            else:
                result.volume_status = VolumeStatus.HEAVY_VOLUME_DOWN
                result.volume_trend = "High-volume drop, caution advised"
        elif result.volume_ratio_5d <= self.VOLUME_SHRINK_RATIO:
            if price_change > 0:
                result.volume_status = VolumeStatus.SHRINK_VOLUME_UP
                result.volume_trend = "Low-volume rise, upward momentum insufficient"
            else:
                result.volume_status = VolumeStatus.SHRINK_VOLUME_DOWN
                result.volume_trend = "Shrink-volume pullback, washout pattern (positive)"
        else:
    ...
    def _analyze_macd(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if is_golden_cross and curr_zero > 0:
            ...
            result.macd_signal = "⚡ DIF crosses above zero, trend strengthening"
            ...
            result.macd_signal = "⚠️ DIF crosses below zero, trend weakening"
        elif result.macd_dif > 0 and result.macd_dea > 0:
            result.macd_status = MACDStatus.BULLISH
            result.macd_signal = "✓ Bullish MACD, sustained uptrend"
        elif result.macd_dif < 0 and result.macd_dea < 0:
            result.macd_status = MACDStatus.BEARISH
            result.macd_signal = "⚠ Bearish MACD, sustained downtrend"
        else:
            result.macd_status = MACDStatus.BULLISH
    ...
    def _analyze_rsi(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if rsi_mid > self.RSI_OVERBOUGHT:
            ...
            result.rsi_signal = f"⚠️ RSI overbought ({rsi_mid:.1f} > 70), short-term pullback risk elevated"
        elif rsi_mid > 60:
            result.rsi_status = RSIStatus.STRONG_BUY
            result.rsi_signal = f"✅ RSI strong ({rsi_mid:.1f}), bullish momentum sufficient"
        elif rsi_mid >= 40:
            result.rsi_status = RSIStatus.NEUTRAL
            result.rsi_signal = f" RSI neutral ({rsi_mid:.1f}), ranging/consolidating"
        elif rsi_mid >= self.RSI_OVERSOLD:
            result.rsi_status = RSIStatus.WEAK
            result.rsi_signal = f"⚡ RSI weak ({rsi_mid:.1f}), watch for a bounce"
        else:
            result.rsi_status = RSIStatus.OVERSOLD
            result.rsi_signal = f"⭐ RSI oversold ({rsi_mid:.1f} < 30), high probability bounce"
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if bias < 0:
            ...
            if bias > -3:
                ...
                score += 16
                reasons.append(f"✅ Price pulling back to MA5 ({bias:.1f}%), observe support")
            else:
                score += 8
                risks.append(f"⚠️ Deviation too large ({bias:.1f}%), potential breakdown")
        elif bias < 2:
            score += 18
            reasons.append(f"✅ Price near MA5 ({bias:.1f}%), good entry opportunity")
        elif bias < base_threshold:
            score += 14
            reasons.append(f"⚡ Price slightly above MA5 ({bias:.1f}%), small position acceptable")
        elif bias > effective_threshold:
            score += 4
            risks.append(
                f"❌ Bias rate too high ({bias:.1f}% > {effective_threshold:.1f}%) — do not chase high!"
            )
        elif bias > base_threshold and is_strong_trend:
            score += 10
            reasons.append(
                f"⚡ Bias elevated in strong trend ({bias:.1f}%), track with small position"
            )
        else:
            score += 4
            risks.append(
                f"❌ Bias rate too high ({bias:.1f}% > {base_threshold:.1f}%) — do not chase high!"
            )
        ...
        if result.volume_status == VolumeStatus.SHRINK_VOLUME_DOWN:
            reasons.append("✅ Shrink-volume pullback, washout pattern")
        elif result.volume_status == VolumeStatus.HEAVY_VOLUME_DOWN:
            risks.append("⚠️ High-volume drop, risk alert")

        # === Support scoring (10 pts) ===
    ...
    def format_analysis(self, result: TrendAnalysisResult) -> str:
        ...
        lines = [
            ...
            f"",
            f"📊 Trend Status: {result.trend_status.value}",
            ...
            f"   Vol Ratio (5d avg): {result.volume_ratio_5d:.2f}",
            ...
            f"",
            f"🎯 Action Signal: {result.buy_signal.value}",
